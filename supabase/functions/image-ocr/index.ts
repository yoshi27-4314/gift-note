...
- 会社名がない個人の名刺の場合は type を "personal" にしてください`;
      ...
      systemPrompt = "あなたは画像から商品・アイテム情報を読み取るOCRアシスタントです。バーコード画像にも対応します。";
      userPrompt = `この画像を分析してください。バーコード（JANコード/EANコード/QRコード）が写っている場合はそのコードを読み取り、商品画像やスクリーンショットの場合は商品情報を読み取ってください。

必ず以下のJSON形式のみで回答してください。他のテキストは含めないでください。
{
  "title": "商品名・アイテム名（正式な製品名をできるだけ正確に）",
  "brand": "ブランド名・メーカー名（読み取れれば）",
  "barcode": "バーコード番号（JANコード等が読み取れた場合。なければnull）",
...
注意:
- バーコードが写っている場合：バーコード番号を正確に読み取り、barcodeフィールドに記載。商品名がわかればtitleにも記載
- 商品画像の場合：商品名は画像から読み取れる正式な名称を使ってください（型番やシリーズ名も含む）
