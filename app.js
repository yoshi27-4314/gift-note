...
- 幅広いジャンルから3つ選んでください`;
  ...
  try {
    const result = await callAI(prompt, like ? '好きなモノ: '+like : '', true, true);
...
async function callAI(prompt, context, structured, skipAuth) {
  ...
  const res = await fetch(AI_EDGE_FN, {
    ...
    body: JSON.stringify({ message: prompt, context: context || '', structured: !!structured, tone: skipAuth ? '' : getAwaiTonePrompt() })
  });
  if (!res.ok) {
