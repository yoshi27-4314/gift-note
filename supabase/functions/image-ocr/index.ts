...
必ず以下のJSON形式のみで回答してください。他のテキストは含���ないでください。
{
  "title": "商品名・ア���テム名",
  "category": "以下のカテゴリから最も適切な1つを選択: ファッション, グルメ, お菓子, 美容・健康, インテリア, 家電・ガジェット, 趣味・体験, ギフト券, その他",
  "genres": ["以下のジャンルから該当するものを配列で（複数可）: 服,靴,バッグ,アクセサリ��,時計,帽子,財布,ストール,ネクタイ,サングラス,お酒,コーヒー,紅茶,調味��,お取��寄せ,フルーツ,ワイン,日本酒,焼酎,チョコレート,焼き菓子,和菓子,ケーキ,クッキー,ゼリー,アイス,おせんべい,ナッツ,ギフ���ボックス,コスメ,オイル,スキンケア,香水,ヘアケア,入浴剤,サプリ,マッサージ,アロマ,雑貨,キッチン用品,食器,花,観��植物,キャンドル,タオル,寝具,文房具,イヤホン,スピーカー,充電器,スマホケース,カメラ,家電,ゲーム,本,音楽,映画,チケット,体験ギフト,習い��,旅行券,スポーツ用品,Amazonギフ���,商品券,カタロ��ギフト,QUO��ード,食事券"],
  "price": "価格が読み取れれば数値（円）、なければnull",
  "memo": "その他読み取れた情報（ブランド名、説明文など）"
}

注意:
- 商品名は画像から読み取れる正式な名称を使ってください
- カテゴリは必ず指定リストから1つ選んでください
- ジャンルは指定リストから該当するものを選んでください（なければ空配列）
- 価格が見える場合は数値のみ（カンマや円マークは除く）
- 読み取れない項目はnullにしてください`;
    } else if (mode === "place_ocr") {
      systemPrompt = `あ���たは画像から場所・店舗情報を読み取るOCRアシスタントです。`;
      userPrompt = `この画像に写っている場所・お��の情報を読み取ってください���

必ず以下のJSON���式のみで回答してください。他のテキストは含めないでください。
{
  "title": "場所名・店名",
  "category": "以下のカテゴリから最も適切な1つを選択: 食事, 遊び, 観光, 買��物, 宿泊, リラックス, そ��他",
  "genres": ["以下のジャンルから該当するものを配列で（複数可）: 和食,洋食,イタリアン,フレンチ,中華,焼肉,寿司,ラーメン,カレー,居酒屋,バー,カフェ,パン屋,スイーツ,ビュッフェ,鉄板焼き,海鮮,うどん・そば,カラオケ,ボウリング,テーマパーク,ゲームセンター,映画館,ライブ,スポーツ観戦,釣��,キャンプ,BBQ,ドラ���ブ,プール,スキー,ゴルフ,神社,お寺,温泉,絶景,公園,美術館,博物館,水族館,動物園,城,庭園,展望台,街歩き,離島,世���遺産,ショッピングモール,アウトレット,商店街,百貨店,セレクトショップ,お土産,市場,���の市,ホテル,旅館,民宿,グランピング,コテージ,ゲストハウス,ス��,マッサージ,エステ,サウ��,岩盤浴,ヨガ,リトリート"],
  "address": "住所が読み取れれば",
...
- カテゴリは必���指定リストから1つ選んでくだ��い
- ジャンル��指定リストから該当するも���を選んでください（なければ空配列）
- 読み取れない項目はnullにしてください`;
    } else {
      return new Response(
